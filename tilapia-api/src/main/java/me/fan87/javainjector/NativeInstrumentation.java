...
public class NativeInstrumentation implements Instrumentation {
    ...
    public synchronized static void loadNativeLib() throws IOException {
        String extension = "";
        byte[] bytes;
        if (OSUtils.isWindows()) {
            extension = ".dll";
            bytes = readAllBytes(Objects.requireNonNull(TilapiaCore.Companion.getInstance().getPlugin().getResource("native-agent-native.dll")));
        } else {
            extension = ".so";
            bytes = readAllBytes(Objects.requireNonNull(TilapiaCore.Companion.getInstance().getPlugin().getResource("libnative-agent-native.so")));
        }
