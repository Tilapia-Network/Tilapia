...
public class JSON2NBT {
    ...
    private static Any nameValueToNBT(String key, String value) throws NBTException {
        ...
        if (value.startsWith("{")) {
            ...
            Compound JSON2NBT$list1;
            for (JSON2NBT$list1 = new Compound(key); value.length() > 0; value = value.substring(s.length() + 1)) {
            ...
            List JSON2NBT$list;
            for (JSON2NBT$list = new List(key); value.length() > 0; value = value.substring(s.length() + 1)) {
    ...
    private static class List extends Any {
        ...
        public Tag parse() throws NBTException {
            ...
            while (var2.hasNext()) {
                Any JSON2NBT$any = (Any) var2.next();
